    ...
    func onAppear() {
        ...
        if let entitlementTeamId {
            ...
            if let keyChainAccessGroups = entitlementDict["keychain-access-groups"] as? Array<String> {
                var notFound = true
                if keyChainAccessGroups.contains("\(entitlementTeamId).com.hss.livecontainer.shared") {
                    notFound = false
                    for i in 1..<SharedModel.keychainAccessGroupCount {
                        if !keyChainAccessGroups.contains("\(entitlementTeamId).com.hss.livecontainer.shared.\(i)") {
                            notFound = true
                            continue
                        }
                    }
